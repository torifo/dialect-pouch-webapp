...
- **Always** be mindful of where to place routes. `phx.gen.auth` creates multiple router plugs and `live_session` scopes:
  ...
  - A `live_session :current_admin` scope - for routes that need the current admin but don't require authentication, similar to `:fetch_current_scope_for_admin`
  - A `live_session :require_authenticated_admin` scope - for routes that require authentication, similar to the plug with the same name
...
- To derive/access `current_admin` in templates, **always use the `@current_scope.admin`**, never use **`@current_admin`** in templates or LiveViews
- **Never** duplicate `live_session` names. A `live_session :current_admin` can only be defined __once__ in the router, so all routes for the `live_session :current_admin`  must be grouped in a single block
...
LiveViews that require login should **always be placed inside the __existing__ `live_session :require_authenticated_admin` block**:
    ...
    scope "/", AppWeb do
      ...
      live_session :require_authenticated_admin,
        ...
        # our own routes that require logged in admin
...
LiveViews that can work with or without authentication, **always use the __existing__ `:current_admin` scope**, ie:
