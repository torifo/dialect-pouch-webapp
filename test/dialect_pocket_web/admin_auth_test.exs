defmodule DialectPocketWeb.AdminAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, DialectPocketWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_admin/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      DialectPocketWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        DialectPocketWeb.Endpoint.subscribe("admins_sessions:#{Base.url_encode64(token)}")
      end
