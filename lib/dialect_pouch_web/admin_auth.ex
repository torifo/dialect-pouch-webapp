defmodule DialectPouchWeb.AdminAuth do
  ...
  def log_out_admin(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      DialectPouchWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      DialectPouchWeb.Endpoint.broadcast(admin_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule DialectPouchWeb.PageLive do
        use DialectPouchWeb, :live_view

        on_mount {DialectPouchWeb.AdminAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{DialectPouchWeb.AdminAuth, :require_authenticated}] do
