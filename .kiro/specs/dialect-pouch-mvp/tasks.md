...
- [ ] **Task 1.1**: プロジェクト scaffold & 基盤整備
  - What: `mix phx.new dialect_pouch`（LiveView/Tailwind 有効)。deps 追加（`oban`, `geo_postgis`, `hammer`)。DB拡張マイグレーション（`ltree`, `postgis`, `pg_bigm` の `CREATE EXTENSION`)。Ltree/Geo の Ecto カスタム型を配線。`Oban` を supervision tree に登録。
  - Files: `mix.exs`, `config/*.exs`, `priv/repo/migrations/*_enable_extensions.exs`, `lib/dialect_pouch/application.ex`
...
- [ ] **Task 2.1**: Regions コンテキスト
  ...
  - Files: `lib/dialect_pouch/regions/*`, `priv/repo/migrations/*_create_regions.exs`
...
- [ ] **Task 2.2**: Accounts（管理者認証)
  ...
  - Files: `lib/dialect_pouch/accounts/*`, `lib/dialect_pouch_web/.../admin_*`, `router.ex`
...
- [ ] **Task 3.1**: Dictionary スキーマ & コンテキスト
  ...
  - Files: `lib/dialect_pouch/dictionary/*`, `priv/repo/migrations/*`
...
- [ ] **Task 4.1**: 検索（API + SearchLive)
  ...
  - Files: `lib/dialect_pouch/dictionary/search.ex`, `lib/dialect_pouch_web/live/search_live.ex`
...
- [ ] **Task 4.2**: エントリ/地域ページ + 地図ドリルダウン
  ...
  - Files: `lib/dialect_pouch_web/live/entry_live.ex`, `region_live.ex`, `components/map_component.ex`, `components/provenance_badge.ex`
...
- [ ] **Task 4.3**: 変換（語/言い回し + 文章内置換)
  ...
  - Files: `lib/dialect_pouch/conversion/*`, `lib/dialect_pouch_web/live/convert_live.ex`
...
- [ ] **Task 4.4**: 投稿（ContributeLive)
  ...
  - Files: `lib/dialect_pouch/contributions/*`, `lib/dialect_pouch_web/live/contribute_live.ex`
...
- [ ] **Task 4.5**: 取り込みパイプライン（Oban)
  ...
  - Files: `lib/dialect_pouch/ingestion/*`, `lib/dialect_pouch/ingestion/workers/*`
...
- [ ] **Task 5.1**: モデレーション（admin LiveView)
  ...
  - Files: `lib/dialect_pouch_web/live/admin/moderation_live.ex`, `lib/dialect_pouch/contributions/moderation.ex`
...
- [ ] **Task 5.2**: SEO 仕上げ（sitemap + メタ)
  ...
  - Files: `lib/dialect_pouch_web/controllers/sitemap_controller.ex`, `components/seo.ex`, `ingestion/workers/sitemap_worker.ex`
...
- [ ] **Task 6.1**: E2E 統合テスト（5シナリオ)
  ...
  - Files: `test/dialect_pouch_web/integration/*`
...
- [ ] **Task 6.2**: 配信（mix release + 運用)
  ...
  - Files: `rel/*`, `Dockerfile`, `deploy/nginx.conf`, `deploy/dialect_pouch.service`, `README.md`(運用節)
